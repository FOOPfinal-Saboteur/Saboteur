	...
	public void setHandler(MapGUI map, ArrayList<CardGUI> gui, Shuffler shuffler, Pane pane) {
		...
		this.setOnMouseDragged(new EventHandler<MouseEvent>() {
			public void handle(MouseEvent me) {
				...
				if (me.getSceneX() >= map.getLayoutX() + (MapGUI.coordStartX + MapGUI.coordDistanceX * i) * map.getRatio() &&
					...
					Main.gx = i; Main.gy = j;
					if ( (!map.isOccupied(i, j) && !CardGUI.this.isFunc && map.canLegallyPlace(CardGUI.this, i, j))||
		...
		this.setOnMouseReleased(new EventHandler<MouseEvent>() {
			public void handle(MouseEvent me) {
                ...
                if ((!map.isOccupied(i, j) && map.isGreen(i, j) && map.canLegallyPlace(CardGUI.this, i, j) )
